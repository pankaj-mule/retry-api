...
const customizationConfigPath = path.join(__dirname, 'config', 'customization-config.json');
const customizationConfig = JSON.parse(
  fs
    .readFileSync(customizationConfigPath, 'utf8')
    .split('\n')
    .filter((line) => !line.trim().startsWith('// Generated by AI'))
    .join('\n')
);

// Static config endpoint for solution customizations
app.get('/sdpms/api/v1/solutions/:solutionId/customization/config', (req, res) => {
  res.status(200).json(customizationConfig);
});
// Generated by AI - END
